...
class Data:
    ...
    def fant_raw(self, year=THIS_YEAR):
        FANT = f'https://www.pro-football-reference.com/years/{year}/fantasy.htm'
        r = requests.get(FANT)
    ...
    def career_stats(self, player_name, advanced=False):
        ...
        if player_name in player_dict:
            if advanced is True:
                prof_link = BASE_URL + player_dict[player_name][:-4] + '/gamelog/advanced'
            else:
                prof_link = BASE_URL + player_dict[player_name][:-4] + '/gamelog/'
        else:
